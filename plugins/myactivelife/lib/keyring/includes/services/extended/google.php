...
class Keyring_Service_Google extends Keyring_Service_OAuth2 {
	const NAME        = 'google';
	const LABEL       = 'Google';
	const SCOPE       = 'https://www.googleapis.com/auth/plus.login'; // See https://developers.google.com/google-apps/contacts/v3/#authorizing_requests_with_oauth_20
	...
	function __construct() {
		...
		$this->set_endpoint( 'self',         'https://www.googleapis.com/userinfo/v2/me', 'GET'  );
	...
	function build_token_meta( $token ) {
		...
		if ( !Keyring_Util::is_error( $response ) ) {
			...
			$meta['profile']  = $response->link;
			$meta['picture']  = $response->picture;
		}

		return apply_filters( 'keyring_access_token_meta', $meta, 'google', $token, $response, $this );
	}
...
add_action( 'keyring_load_services', array( 'Keyring_Service_Google', 'init' ) );
